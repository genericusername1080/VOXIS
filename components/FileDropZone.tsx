...
export const FileDropZone: React.FC<FileDropZoneProps> = ({
  ...
  return (
    ...
    >
      <BauhausCard
        className={`w-full max-w-2xl min-h-[400px] flex flex-col items-center justify-center text-center transition-transform duration-300 ${isDragging ? 'scale-105 rotate-1' : ''}`}
        color={isDragging ? 'yellow' : 'white'}
      >
        <div className={`w-24 h-24 mb-8 border-4 border-black rounded-full flex items-center justify-center ${isDragging ? 'bg-[var(--primary-red)]' : 'bg-[var(--primary-blue)]'}`}>
